...
def main() -> None:
    ...
    for day_dir in all_dirs:
        ...
        if not st_path.exists():
            print(f"  Skipping {day_dir.name} — no Sales_Ticket.txt")
            continue
        st = pd.read_csv(st_path, sep="|", dtype=str)
        match = st[st["Ticket_Number"].astype(str).str.strip() == ticket_number].copy()
        if not match.empty:
    ...
    out_path = Path(f"sales_ticket_{ticket_number}_{start_date}_{end_date}.csv")
